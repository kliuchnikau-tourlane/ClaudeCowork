# Email to Charlie — Expectations for Monday materials & Tuesday sync

**To:** charlie@11infotech.com
**Cc:** claudia.tita@tourlane.com, talha.mahmood@tourlane.com, pedro.alves@tourlane.com, nishant.tomer@tourlane.com
**Subject:** Materials needed by Monday for our Tuesday sync

---

Hi Charlie,

Thanks for the discussion yesterday. To prepare for Tuesday, we'd like to ask your team to write down their responses by end of Monday so we can pre-read them. Given how many topics we are covering, having things in writing helps us be better informed going into the meeting and makes sure we don't misunderstand each other along the way.

**What we'd like by Monday — please pick one of the two:**

- **Option A — New API specification.** If your recommendation is to move to a new API, please share the draft specification: what is already defined at this stage, what still needs clarification, and how you propose we approach the data model.
- **Option B — Written answers to our 22-point document, focusing on items 1–14** (the "critical" and "important" sections). For each point, your proposed approach plus any open questions or constraints from your side is enough.

If you don't manage to cover every item in 1–14, that's fine — we'll work through the remaining ones on Tuesday. The important thing is that we have your responses in writing, not just verbal, so we share the same understanding before the meeting.

If anything is unclear in the meantime, please reach out via chat or email — happy to answer ahead of Monday. Just a heads-up: tomorrow (Friday) is a public holiday on our side, so we'll be available today and again on Monday.

To avoid any confusion: the Tourlane API specification we shared is a reference point for our discussion, not a 1:1 implementation requirement.

**On Tuesday**

We'll walk through your written response, discuss the open points, align on the data model, and confirm next steps.

Thanks,
Aliaksei
