...
Usage:
    python3 granola-export.py --date today --output ~/Projects/ClaudeCowork/MeetingNotes/
    python3 granola-export.py --date 2026-04-07 --output ~/Projects/ClaudeCowork/MeetingNotes/
"""
...
def format_timestamp(ts):
    ...
    Handles: ISO strings, numeric seconds/milliseconds, or None.
    """
    if ts is None or ts == "" or ts == 0:
        return ""

    # If it's a string, try parsing as ISO datetime or numeric string
    ...
    # Now ts should be numeric
    ts = float(ts)
    # If timestamp looks like milliseconds, convert to seconds
    if ts > 1_000_000_000_000:
        ts = ts / 1000
    # If it's a Unix timestamp (large number), format as clock time
    if ts > 86400:
        try:
            ...
            return ""
    # Otherwise treat as offset in seconds
    hours = int(ts // 3600)
...
def build_transcript_text(transcript_segments):
    ...
    for seg in sorted(transcript_segments, key=lambda s: s.get("start_timestamp", 0)):
        ...
        ts = format_timestamp(seg.get("start_timestamp"))
...
def main():
    ...
    parser.add_argument(
        ...
        help="Date to export: 'today', 'yesterday', or YYYY-MM-DD (default: yesterday)"
    )
    parser.add_argument(
        "--output", required=True,
        help="Output directory (e.g. ~/Projects/ClaudeCowork/MeetingNotes/)"
    )
    parser.add_argument(
        "--cache", default=None,
